...
2|412|500|100|400`)).toEqual([]);
  ...
  test("★★ 四个数与颜色之间只有空格（2026-09-06 实拍：整帧因此被扔、一段认不出人）也读得出来", () => {
    // 模型原文：`3|422 594 573 998 红色|左手凑到嘴边|在画面中心位置`——数对、人全，只是颜色紧跟在数后面没有 `|`
    const rows = blockout.parseRosterBoxes(
      ["1|422 594 573 998 红色|左手凑到嘴边|在画面中心位置", "2|774 266 245 520 纯白|右手握拳前伸|在红色人偶右后方"].join("\n"),
    );
    expect(rows.map((r) => r.cx)).toEqual([422, 774]);
    expect(rows[0]).toMatchObject({ cy: 594, w: 573, h: 998, color: "红色", act: "左手凑到嘴边" });
    expect(warnSpy.mock.calls.flat().join(" ")).not.toMatch(/像人却读不出来/);
  });
  ...
  test("★★ 纯方位的「位置关系」当成无参照物丢掉（它只是把序号换个说法）", () => {
    // ★★ 2026-08-18 一发实拍里模型就是这么返的（同一段素材前两发给的是
    //   「在最右那盏路灯下」这种真景物）。不接住的话它会白占 34 字预算里的一大半，
    //   把真正有区分力的**动作**挤出去。
    // ★ 词表是实拍一次扩一次（`在画面中心位置` 就是第三发空跑时漏掉的 —— 「心」不在表里）
    for (const w of [
      "左数第二个位置",
      "画面最左侧",
      "画面正中间",
      ...
      expect(blockout.composeRosterDesc({ color: "纯白", act: "蹲姿", where: w })).toBe("纯白、蹲姿");
    }
    // ★ 含方位词、但**确实点到了东西**的不允许误杀
    for (const w of [
      "在最右那盏路灯下",
      "在左数第二条白条纹左侧",
      "贴着中间那根柱子",
      ...
      expect(blockout.composeRosterDesc({ color: "纯白", act: "蹲姿", where: w })).toBe(`纯白、蹲姿、${w}`);
    }
  });

  test("★ 认人提示词明令“只说方位不算”", () => {
    expect(blockout.rosterBoxPrompt()).toMatch(/只说方位不算/);
  });
