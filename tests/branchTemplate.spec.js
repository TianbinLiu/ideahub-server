...
describe("画面位置框（单元）—— 拖到画面上挂卡的全部输入", () => {
  ...
  test("★★ 相邻两个框套在一起 → 整份丢（同一个人被数了两遍）", () => {
    // ⚠ 下面这一组是 2026-08-17 拿**线上真模板**（「编号验证-勿用」）跑出来的真实回包。
    //   数目"对得上"（要 2 给 2），但第 2 个框的中心 621 就落在第 1 个框（280..664）里面，
    //   而且宽只有 71 —— 那不是并排的第二个人，是同一个人的胳膊/影子被当成了人。
    //   这一条判据就是被这一发逼出来的（在它之前，这份坏数据会被放行）。
    const nested = ["1|472|524|384|886", "2|621|675|71|630"].join("\n");
    expect(blockout.parseBoxes(nested, 2)).toEqual([]);

    // ★ 反向钉子：**并排站近、肩膀重叠**是常态，不许把好数据一起判掉。
    //   这一组同样是真实回包（模板「头部编号验证」，5 个人偶）——相邻框互相蹭着，
    //   但每个**中心**都在邻居框外。它必须继续通过。
    const sideBySide = [
      "3|188|593|227|785",
      "2|349|573|201|808",
      "1|521|589|296|831",
    ...
    expect(blockout.parseBoxes(sideBySide, 5)).toHaveLength(5);
    // 顺带钉住"序号列不参与排序"：上面那份的首列是 3/2/1/4/5，排完必须按 cx 升序
