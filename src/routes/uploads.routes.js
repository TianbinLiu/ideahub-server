...
//   一发就是 20MB 级出网 + 永久占 Cloudinary 配额，而且每个热门模板此后每被套用一次，
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      //   提前量，改一行前端就能跳过）。窗口的唯一实现在 middleware/upload.js 的
      //   templateVideoIssue（建模板那一步复核用的是同一份）。
...
// ★ 归属判据 = public_id 前缀（上传时就是 `${userId}-${ts}`，/media 同款），
//   不查任何客户端报的其他数；已被登记的资产整句拒 —— 那些的生命周期归
//   DELETE /api/branch/templates/:id 级联管，从这里删等于把别人在用的模板抽底。
router.delete("/template-video", requireAuth, uploadLimit, async (req, res, next) => {
  try {
    const publicId = String(req.body?.publicId ?? req.query?.publicId ?? "").slice(0, 300);
    const expectedPrefix = `ideahub/template-videos/${req.user._id.toString()}-`;
    if (!publicId.startsWith(expectedPrefix) || !/^[\w/-]+$/.test(publicId)) {
      return res.status(400).json({ ok: false, message: "只能回收本账号上传的模板视频。" });
    }
    const registered = await BranchTemplate.exists({ "refVideo.cloudinaryPublicId": publicId });
    if (registered) {
