...
router.post("/events/simulate", requireAuth, aiRateLimit({ max: 15, scope: "ai:stance-simulate" }), validate({ body: simulateBody }), simulateEvent);
router.post("/ingest", requireAuth, validate({ body: ingestBody }), ingestEvent);
router.post("/events/:id/regenerate", requireAuth, aiRateLimit({ max: 15, scope: "ai:stance-regen" }), regenerateReply);
