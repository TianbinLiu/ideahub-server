...
// ── 白模 V2：角色位 ───────────────────────────────────────────────────
// 「白模人偶头上那个编号 ↔ 原视频里的哪个人」（2026-08-15 起编号印在**头部四面**，
// 不再印胸口 —— 胸口那个一转身就看不见了，而编号是挂卡的唯一连接键，
// 出处 services/blockoutize.blockoutPrompt）。**缺省 = V1 老模板**（整段只有一个
...
const roleSchema = new mongoose.Schema(
  {
     ...
     *   清单顺序编的猜测**（1..N），而成片上人偶头上的数字**不保证连续**（实出 1/2/4/5）。
...
// ★ 这句话要指到**用户真能看到号的那个部位**。2026-08-15 编号从胸口挪到了头部四面
//   （blockoutPrompt 的 ★★①），这句还写"胸口"的话，作者会盯着一个空白的胸口
//   核对不下去，然后以为是生成坏了 —— 一句过时的指路和一个坏功能长得一模一样。
branchTemplateSchema.statics.ROLES_CONFIRM_HINT =
  "请先核对每个角色位的编号与画面上人偶头上的数字是否一致（编号印在人偶头部的前后左右四面，转到哪一面都是同一个号）：" +
