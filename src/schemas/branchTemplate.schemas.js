...
//   一律不收 roles，收了就等于让提交方自己写"绿色位是谁"。
//
// ★★ `markColors`（标记方案位）**三条 body 一律刻意不收**，与 roles/source 同一条理由，
//   而 `patchRolesBody` 那条尤其要害：**让作者改得动方案位 = 让他把一个颜色模板标成
//   编号模板**，套用侧当场整份错（输入框里冒出 `编号绿色=凛`），且零报错。
//   这一位记的是"当初那段视频上究竟印了什么"——是历史事实，不是作者的意见，
...
const roleItemBody = z.object({
  // 标记本身（颜色模板是色名「绿色」，老模板是数字「1」）。
  // ★ maxlength 与 models/BranchTemplate.roleSchema 的 8 对齐，**换颜色时一个字没改**：
  //   调色板的色名一律 2 个汉字，正是照着这个 8 定的（MARK_PALETTE 选色规则⑦）。
