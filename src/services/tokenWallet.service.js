...
async function writeEntry(userId, delta, reason, balanceAfter, memo = "") {
  if (!delta) return; // 0 变动不写流水（月度刷新可能正好等额）
  try {
    await TokenLedger.create({ user: userId, delta, reason, balanceAfter, memo: memo.slice(0, 200) });
  } catch (e) {
...
async function listLedger(userId, limit = 50) {
  return TokenLedger.find({ user: userId })
    ...
    .select("delta reason balanceAfter memo createdAt")
    .lean();
}
