 ...
 *   ⇒ 所以这两个缺陷的退路是**产品级**的，不是提示词级的：作者对着画面把号改对、
 *     把画面上找不到的位删掉（剩下的号一个不变），外加"角色位越少越准"写进上传引导。
 */
function blockoutPrompt(roles) {
  ...
  return [
    ...
    // ★★ 编号段**压到两行，且编号本身「从清单抄」而不是让模型自己编**（见函数头 ★★③④）。
    //   要害四件一件不少：
    //   ① **号从点名清单来**。这是 ④ 那一发的教训：写成「给每个人偶编上 1 到 N 的编号」
    //      等于交给模型一件独立的分配活儿，它就自己配了一套（实测印出 2/2/1/1/5 ——
    //      重号又漏号）。清单里 `3=红双马尾` 本来就把号和人绑好了，让它照抄即可。
    //   ② 取值范围钉死（`1 到 N`＋"不要多位数"）—— 2026-08-15 实测，只说「依次编号」
    //      模型会印出 **115 / 116** 这种三位数，而我们登记的是 1/2，两边完全对不上：
    //      套用者点「1」挂卡，AI 到画面上找不到「1」，钱花了人没换对，且全程零报错；
    //   ③ "**同一个**数字印四处"要说死，不然会变成四个不同的号；
    //   ④ "身上别处不出现数字"，否则模型顺手在胸口也印一个 —— 两处不一致时没人能仲裁
    //      （这也正是胸口那一处被故意去掉的理由，见函数头 ★★①）。
    "点名清单里的编号就是要印的号：清单写几号的那个人，他的人偶头上就印几号，一个号只给一个人偶，不许重复也不许漏号。",
    `号印在头部前后左右四面，四处都是**同一个**醒目的黑色阿拉伯数字（转向哪一边都是这一个号），只用 1 到 ${roles.length}、不要多位数，身上别处不要有数字或文字。`,
    ...
    // ★ 这一条也压成一行：不是它不重要，而是**总长度是有预算的**（函数头 ★★③）——
    //   编号段那两句加长了多少，就要在别处还回去，否则又要拿抹外观那几句去换。
    "清单之外若还出现别的人物（中途入场的、被挡住又露出来的），同样换成一模一样的白色人偶，但**不给编号**，头上身上保持空白。",
