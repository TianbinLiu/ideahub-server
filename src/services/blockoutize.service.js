...
function parseBoxes(text, expected) {
  ...
  for (const rawLine of String(text || "").split(/\r?\n/)) {
    ...
    // ★ 四个数之间允许空格/逗号（与 parseRosterBoxes 同一条宽容，理由写在那边）
    const m = rawLine.trim().match(/^(\d+)\s*[|｜]\s*(-?\d+)(?:\s*[|｜,，]\s*|\s+)(-?\d+)(?:\s*[|｜,，]\s*|\s+)(-?\d+)(?:\s*[|｜,，]\s*|\s+)(-?\d+)\s*$/);
...
function rosterBoxPrompt() {
  return [
    ...
    "  这一项必须点到一个**看得见的东西**（灯、柱子、门、台阶、地上的斑马线…）。",
    "  ⚠ 只说方位不算（「画面最左侧」「左数第二个位置」「靠近中间」都不算）——那只是把序号换个说法，",
    "    对认人没有任何帮助。画面里确实找不到可参照的东西就写「无参照物」，**不要编，也不要用方位凑数**。",
...
/**
 * **纯方位**的"位置关系"——它把序号换了个说法，对认人零帮助，当成「无参照物」处理。
 *
 * ★★ 这是一道**兜底**，不是判据：提示词里已经明说「只说方位不算」，但 2026-08-18 那一发
 *   同一段素材照样回了「左数第二个位置」「画面最左侧」（前两发给的是「在最右那盏路灯下」
 *   「在左数第二条白条纹左侧」这种真景物）。模型不听的时候得有人接住 ——
 *   不接的话，这一截会白占 34 字预算里的一大半，把真正有区分力的**动作**挤出去。
 * ★★ 判据是「**整句只由方位词构成**」，不是"匹配某几种公式化写法"（第一版那么写，
 *   `画面正中间` 当场就漏了 —— 穷举写法这条路本来就走不通）。反过来想才对：
 *   一句**真的点到了东西**的描述必然含有方位词之外的字（灯、柱、门、条纹、台阶…），
 *   而纯方位句从头到尾都在这个小词表里。于是"漏掉一种没见过的退化写法"变成了
 *   结构上不可能，而**误杀**（把真景物当成纯方位丢掉）同样不可能。
 * ★ 词表里刻意**不含**任何名词：「中间」「正中」都是方位，「柱子」「路灯」不是。
 */
const BARE_POSITION_RE =
  /^[在位于画面镜头最正靠近中间央心左右上下前后方侧边端角排列半里数第一二三四五六七八九十个的位置向偏处0-9\s]+$/;
...
function composeRosterDesc({ color, act, where }) {
  ...
  // ★ 两道都是"这一项没有信息"：模型自己说的「无参照物」，以及它没听话、拿方位凑数的那种
  const w = String(where || "").trim();
  if (w && !NO_ANCHOR_RE.test(w) && !BARE_POSITION_RE.test(w)) parts.push(w);
...
function parseRosterBoxes(text) {
  ...
  for (const rawLine of String(text || "").split(/\r?\n/)) {
    const line = rawLine.trim();
    if (!line || /^NONE$/i.test(line)) continue;
    // ★★ 四个数之间**允许空格或逗号**（2026-09-05 主人真机实拍）：模型把一行写成
    //   `1|96 602 187 703|纯白|身体前倾右臂前伸|踩在左侧黑台子上` —— 数是对的、人也认全了，
    //   只是没按 `|` 分隔。老正则不认，这一行被计成"像人却读不出来"，**整帧丢弃**；
    //   五个候选帧全这么写，这一段就"认不出人"，而模型明明每帧都看见了 10 个人。
    //   序号与描述之间仍然只认 `|`：那两道是列的边界，松了会把描述里的空格也切开。
    const m = line.match(
      // ★★ 第四个数与描述之间也允许只有空格（2026-09-06 主人真机）：模型把颜色紧跟在数后面
      //   `3|422 594 573 998 红色|左手凑到嘴边|在画面中心位置` —— 五行全是这个形状，整帧被扔、这一段"认不出人"。
      //   `(.+)` 兜住描述列，多出来的第五个数（若有）只是落进描述里，不会被读成坐标。
      /^(\d+)\s*[|｜]\s*(-?\d+)(?:\s*[|｜,，]\s*|\s+)(-?\d+)(?:\s*[|｜,，]\s*|\s+)(-?\d+)(?:\s*[|｜,，]\s*|\s+)(-?\d+)(?:\s*[|｜]\s*|\s+)(.+)$/,
    );
    if (!m) {
  ...
  if (bad > 0) {
    console.warn(`[blockoutize] 认人+量框：${bad} 行读不出来或数值不合法，整份丢弃`);
    return [];
  }
  if (!out.length) return [];
  out.sort((a, b) => a.cx - b.cx);
  // ★★ 超过角色位上限的群戏**不按"中心太近"整帧否决**（2026-09-05 主人真机实拍）：
  //   一帧认出 10 个人，第 3、4 个中心只差 29（阈值 33）—— 密集站位、前后遮挡时相邻中心
  //   本来就近，这不是"同一个人数了两遍"。而这条阈值是拿 2~7 人的素材校准的（parseBoxes
  //   那边的回归表），对 10 人以上的群戏没有任何数据支撑。整帧否决的代价是这一段永远
  //   "认不出人"；放行的代价是可能多登记一个重复位 —— 而后者正是核对面板（删掉画面上
  //   不存在的位子）存在的理由，作者一眼就能处理。上限之内照旧否决，回归表一条不动。
  if (out.length > BLOCKOUT_ROLE_MAX) {
    console.warn(
      `[blockoutize] 认人+量框：这一帧认出 ${out.length} 个人（超过上限 ${BLOCKOUT_ROLE_MAX}），` +
        `密集群戏不按中心间距否决，只登记最左 ${BLOCKOUT_ROLE_MAX} 个、交给作者核对`,
    );
    return out;
  }
