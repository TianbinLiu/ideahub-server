...
- `chatSafety.service.js` - 聊天链路的**自伤危机协议**（加州 SB 243 §22602、纽约 GBL §1701/§1702）：归一化（「想 死」「自.杀」这类插字写法照样认）、输入检测（C-SSRS 五类：想死 / 自杀 / 方法 / 自伤）、输出检测（另加鼓动、教方法、骂人「去死」）、按地区给热线（US 988 / 大陆 12356+110/120 / 其它 findahelpline）、`recordReferral` 匿名计数、`PROTOCOL_VERSION`（与官网 /safety/ai-chat 同版本）。刻意排除「我想死你了」这类想念说法；纯正则纯函数，可以在流式回调里逐句跑
