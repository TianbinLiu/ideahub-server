...
- `directUpload.service.js` - **Cloudinary 签名直传「一张票」的唯一实现**（铁律六）：`signDirectUpload`（public_id 服务端签死 / overwrite:false / allowed_formats 三条纪律与它们各自挡住的实测漏洞）、`cloudinaryReady`、`directDeliveryUrl`（公开投递地址，避开 Admin API 的 500 次/小时全局配额）、`DIRECT_UPLOAD_CHUNK_BYTES`。三个上传口共用：模板视频 / 发布成片 / Live2D 模型包
