...
- `SafetyReferralStat.js` - 危机转介的**匿名**计数（day + scene + trigger + region → count）。加州 SB 243 §22603 从 2027-07-01 起要报「发出转介多少次」，且报告不得含用户标识 —— 所以这张表里永远不能出现 user / thread / ip / 原文，有测试盯着字段集合
- `TakedownRequest.js` - **NCII 移除请求**（TAKE IT DOWN Act §3）：免登录提交，`dueAt = receivedAt + 48h` 由 pre hook 推导（不接受调用方传值），`removed[]` 记下实际移除了哪些（含已知相同副本）。**不随删号级联清掉、不设 TTL**：它是我们「在 48 小时内处理过」的唯一证据
- `ChatThread.js` - 数字人对话的一个会话（scene = companion 首页陪聊 | support App 客服，两边互不可见）：标题、原子序号 `seq`、`lastActiveAt`（保留期从这里算：陪聊 180 天、客服 30 天，惰性清扫）、滚动摘要 `summary`（≤600 字；`coversUntilSeq` = 摘要覆盖到哪条，是提纯的提交点）、上下文计量 `stats`（上一轮接口 usage，没给就用校准估算 / 估算校准系数 calibK / 连续提纯失败次数 / 提纯租约 `compacting` + `compactingAt`，5 分钟过期）
- `ChatMessage.js` - 会话里的一条消息：`displayText`（给人看，剥掉演出标签）与 `modelText`（模型原文，带标签，发给模型的历史用它）；`compacted` = 已提纯进摘要、不再发给模型但原文保留给用户翻看；`kind=divider` 是「已整理前 N 轮」分隔提示；`partial` = 回复中途断开存下的半截
- `ChatMemory.js` - 「小梦记得的事」（事实卡）：陪聊跨会话、客服只在本会话内（`sourceThreads`）；可编辑 / 回退上一版 / 置顶 / 删除；删会话时从它提炼的卡一并硬删；不做 TTL
- `ChatUsageLog.js` - 每次调模型的 token 用量（只有数字：prompt / completion / 缓存命中 / 未命中 / 推理），kind = reply | compact；Mongo TTL 180 天
- `DeletionLog.js` - 用户主动删除过什么（只存 ID，TTL 400 天）：备份恢复后按它重放删除，被删的东西不复活
- `CompanionSetting.js` - 每个用户给数字人（官网看板娘 + App 客服，同一形象）做的三项选择：persona（人格市场）/ model（Live2D 模型市场，null = 官方内置）/ voice（豆包音频覆盖：单音色 `voiceId` 或声音市场的混音配方 `mix` + 来源 `templateId`）。persona / model 只存 id 不存快照，被删/取消分享读取时静默回退；voice 的混音配方是**快照**（模板被删只把 `templateId` 置 null，嗓子不变）。`voiceSettingsSchema` / `voiceMixEntrySchema` 子文档被 `Persona.voice` / `Live2dModel.voice` / `VoiceTemplate.recipe` 复用；与 `PersonaEquip`（Arena 发言风格）是两回事
...
- `VoiceTemplate.js` - **声音市场**的混音模板：1～3 味豆包 1.0 音色（`config/voices.js` 的 `MIXABLE_VOICES`，23 个验证过能出声的）按权重调一把嗓子（`recipe` 归一化：和 = 1、三位小数）+ rate / pitch / instruct / expressive、shared、stats(use/like)。「使用」是**快照**语义：配方复制进引用方的 voice，只留 `templateId` 标来源，作者改 / 删都不影响已使用的人
- `VoiceTemplateLike.js` - 声音模板点赞关系，{user,template} 唯一索引
- `Interest.js` - 公司兴趣表达
- `OtpToken.js` - 邮箱验证码
- `AiJob.js` - AI评审任务队列
...
- `branchProject.routes.js` - 已发布作品的「工坊工程」（画布快照），同一个 `/api/branch` base，路径全在 `/projects` 下：`GET /projects`（只回元信息，**绝不回 canvas**）、`GET|PUT|DELETE /projects/by-video/:videoId`（PUT 限流 12/分钟 `branch:project`）。四条全部 requireAuth + 仅作者
- `companion.routes.js` - 首页看板娘数字人（**含自伤危机协议**：输入命中不调模型直接回求助卡、输出侧守卫、`notice` 身份告知、`PUT /consent` 首次同意、`config.safety`）：`GET /config` 游客探测（有没有 AI/TTS、叫什么；登录用户还带自己的 `persona` / `model` / `voiceSettings` 解析结果），`GET|PUT /settings` 数字人三项选择（人格市场的人格 / Live2D 模型市场的模型 / 豆包音频覆盖；人格要能选用：公开或自己的，付费需已购，否则 403 带 `details.reason`），`POST /chat` 登录 + 按用户限流的 **SSE 流式对话**——LLM 按「一句一组 `[情绪][face:x][action:y]` 标签」输出，服务端逐句切分、解析标签后以 `sentence` 事件下发（含给 `/api/tts` 用的情绪参数），前端按句合成语音并切表情/动作。客户端断开即 abort 上游，不白烧 token。**2026-09-18 起**请求体可以是 `{ message, threadId? }`（历史由服务端持有，见 `chatMemory.service`；先发 `thread` 事件，`done` 带 `threadId` 与 `context` 用量），旧写法 `{ messages[] }` 保留给已发布的客户端（不落库）
- `support.routes.js` - App「AI 客服」。**2026-09-25 起同样接了自伤危机协议**（它是 Google Play 上唯一的通用对话入口，同一位看板娘、同一套人格，而「纯客服豁免」还没拍板）：输入命中不调模型直接回求助卡、输出逐句 + 整段复查、接守卫后不再发未经检查的 `token` 事件。（一个文件导出 publicRouter + adminRouter）：`GET /api/support/config`（登录用户同样带 `persona` / `model` / `voiceSettings`，与官网看板娘共用同一份 CompanionSetting）、`POST /api/support/chat`（SSE，请求体与事件都与 companion 相同——含 2026-09-18 起的 `{ message, threadId? }` 按会话写法，多一个 `handoff`——模型在回复开头写 `[handoff:类别]` 即转人工判定；存进历史的原文剥掉 handoff 标记）、`POST /api/support/tickets`（建单 → 通知所有管理员 + 邮件，10 分钟内复用）、`GET /tickets/mine`、`POST /tickets/:id/messages`；管理员 `/api/admin/support/tickets` 列表/详情/回复/改状态（回复与结单会给用户发 SUPPORT_REPLY 通知 + 邮件）。通知与邮件都是尽力而为，失败不影响 201；`POST /feedback` 满意度、管理员 `GET /feedback?rating=`
- `takedown.routes.js` - NCII 移除：`POST /api/takedown`（**免登录**，按 IP 10 次/小时）+ `/api/admin/takedown`（列表 / `:id/scan` 副本检索 / `:id` 处置）。★ 与站内举报是两条通道：举报要登录、指向站内对象；这条免登录、只要一个链接，且有 48 小时法定时限
- `companion.routes.js` - 首页看板娘数字人：`GET /config` 游客探测（有没有 AI/TTS、叫什么；登录用户还带自己的 `persona` / `model` / `voiceSettings` 解析结果），`GET|PUT /settings` 数字人三项选择（人格市场的人格 / Live2D 模型市场的模型 / 豆包音频覆盖；人格要能选用：公开或自己的，付费需已购，否则 403 带 `details.reason`），`POST /chat` 登录 + 按用户限流的 **SSE 流式对话**——LLM 按「一句一组 `[情绪][face:x][action:y]` 标签」输出，服务端逐句切分、解析标签后以 `sentence` 事件下发（含给 `/api/tts` 用的情绪参数），前端按句合成语音并切表情/动作。客户端断开即 abort 上游，不白烧 token。**2026-09-18 起**请求体可以是 `{ message, threadId? }`（历史由服务端持有，见 `chatMemory.service`；先发 `thread` 事件，`done` 带 `threadId` 与 `context` 用量），旧写法 `{ messages[] }` 保留给已发布的客户端（不落库）
- `support.routes.js` - App「AI 客服」（一个文件导出 publicRouter + adminRouter）：`GET /api/support/config`（登录用户同样带 `persona` / `model` / `voiceSettings`，与官网看板娘共用同一份 CompanionSetting）、`POST /api/support/chat`（SSE，请求体与事件都与 companion 相同——含 2026-09-18 起的 `{ message, threadId? }` 按会话写法，多一个 `handoff`——模型在回复开头写 `[handoff:类别]` 即转人工判定；存进历史的原文剥掉 handoff 标记）、`POST /api/support/tickets`（建单 → 通知所有管理员 + 邮件，10 分钟内复用）、`GET /tickets/mine`、`POST /tickets/:id/messages`；管理员 `/api/admin/support/tickets` 列表/详情/回复/改状态（回复与结单会给用户发 SUPPORT_REPLY 通知 + 邮件）。通知与邮件都是尽力而为，失败不影响 201；`POST /feedback` 满意度、管理员 `GET /feedback?rating=`
...
- `tts.routes.js` - 豆包语音合成代理（openspeech V3 SSE），base `/api/tts`：`GET /health`（配没配 key）、`GET /voices`（`{ voices: 2.0 目录(每条 generation:"2.0"/mixable:false), mixable: 23 个 1.0 可混音音色(id/name/gender/generation:"1.0"/mixable:true), defaultVoiceId, maxMixVoices: 3 }`，数据源 `config/voices.js`）、`POST /`（登录 + 30 次/分钟；`mix` 同时接受 `[{ id, w }]`（老）与 `[{ voiceId, weight }]`（VoiceSettings.mix），≤3 味、权重归一到和 = 1，走 `custom_mix_bigtts` + `seed-tts-1.0`；单音色按 uranus 与否选 seed-tts-2.0 / 1.0，`instruct` 只对 2.0 生效）
- `voiceTemplate.routes.js` - **声音市场**（豆包 1.0 混音模板），base `/api/voice-templates`：广场列表（`?page&limit(≤40)&sort=new|hot&q&scope=all|mine` → `{ templates, total, page, limit, totalPages }`，all = 已分享、mine 未登录 401）、详情（私有非作者 403）、创建（登录 + 按用户 10 次/分钟；`{ name(1..60), description?(≤300), recipe[1..3]{voiceId,weight}, rate?, pitch?, instruct?, expressive?, shared? }`，recipe 只收 `MIXABLE_VOICES`，2.0 进来 400 且 message 说明只能混 1.0）、作者改 / 删（删除连点赞一起删，引用它的数字人设置 / 人格 / 模型只把 `templateId` 置 null、配方快照保留）、`POST /:id/like` → `{ liked, likeCount }`、`POST /:id/use` → `{ useCount }`（前端应用模板时调一次）。载荷 `VoiceTemplate = { _id, author{_id,username}, name, description, recipe, rate, pitch, instruct, expressive, shared, stats{useCount,likeCount}, liked, isOwner, createdAt, updatedAt, voice }`，其中 `voice` 是由 recipe/rate/pitch/instruct 拼好、`templateId=_id` 的 VoiceSettings 快照，前端直接塞进 `settings.voice`
- `chatThreads.routes.js` - 数字人对话的会话与记忆（陪聊 + 客服共用，base `/api/chat`，全部需登录、只碰自己的）：`GET /threads?scene=` 会话列表（带 context 用量）、`GET /threads/:id/messages?before=&limit=` 翻历史、`POST /threads/:id/compact {focus?}` 手动「整理记忆」（调模型，按用户 5 次/分钟；正在整理 409、会话已删 404、失败 502）、`DELETE /threads/:id` / `DELETE /threads?scene=`（硬删，连带从它提炼的记忆卡）、`GET|DELETE /memories?scene=`、`PATCH /memories/:id {text?, pinned?}`、`POST /memories/:id/revert`、`DELETE /memories/:id`
...
#### `server/src/services/`
- `email.service.js` - 邮件发送：通用 `sendEmail({to, subject, html, text})`（Resend / dev 打日志）+ `sendEmailOtp`
- `otp.service.js` - OTP生成/验证
- `notification.service.js` - 通知创建
- `aiReview.service.js` - AI评审队列
- `workshopAi.service.js` - 工坊模板与全站编辑 AI 草案生成
- `companion.service.js` - 看板娘数字人的「演出协议」：人设提示词（9 种表情 × 11 种动作的标签规范）、流式切句器（标签跨 chunk / 句尾无标点也能切）、标签解析（未知值回退默认）、情绪 → 豆包 TTS 参数映射。纯函数、无 IO，`tests/companion.spec.js` 直接测；2026-09-05 起还放 `streamCompanionReply`（SSE 流式回复 sentence / token / done / error 的唯一实现，首页看板娘与人格试聊共用；客服链路因 handoff 前缀仍是自己的一份）与 `personaExampleMessages`（人格卡的示例对话 → few-shot 轮次，助手侧补中性演出标签）
- `personaAi.service.js` - 人格向导的 AI：`analyzeMaterials`（素材按段抽样到 12k 字 → 口头禅 / 句长 / 标点 / 语气词 / 立场 / 话题 / 代表性原话）、`generatePersonaDraft`（基本设定 + 问卷 + 分析 / 或老入口的一段聊天 → 人格卡草稿，含 tone / addressUser / greeting / examples / boundaries；`only` + `draft` 只重生成部分字段）；只出草稿不落库，进出文本一律 `scrubPii`（手机号 / 身份证 / 邮箱 / IP）
...
- `chatSafety.service.js` - 聊天链路的**自伤危机协议**（2026-09-25 评审后：方法类词表带意图/动作/量词约束，「打农药」「医生开的安眠药」「跳楼机」「烧炭烤肉」不再误伤；归一化改白名单，emoji / 软连字符 / 字母数字插字都挡得住；`sanitizeHistory` 给旧写法那条不存历史的链路做占位替换）（加州 SB 243 §22602、纽约 GBL §1701/§1702）：归一化（「想 死」「自.杀」这类插字写法照样认）、输入检测（C-SSRS 五类：想死 / 自杀 / 方法 / 自伤）、输出检测（另加鼓动、教方法、骂人「去死」）、按地区给热线（US 988 / 大陆 12356+110/120 / 其它 findahelpline）、`recordReferral` 匿名计数、`PROTOCOL_VERSION`（与官网 /safety/ai-chat 同版本）。刻意排除「我想死你了」这类想念说法；纯正则纯函数，可以在流式回调里逐句跑
- `nciiTakedown.service.js` - NCII 移除请求的**已知相同副本检索**（§3(b)(1)(B)）与到期提醒：`MEDIA_SOURCES` 是「用户上传的影像可能落在哪」的唯一登记表（22 张表；漏登记一处 = 那份副本永远搜不到且零报错，`tests/nciiTakedown.spec.js` 扫模型目录逐字段核对），作品页链接会展开成该作品的全部资产地址再反查，Cloudinary 变换过的地址按尾段文件名也认得出；`sweepDueReminders` 在剩 12 小时与逾期各发一封信（只在 0 号实例跑）。⚠ 与 `utils/branchAssetRefs` 的取舍**刚好相反**：那边刻意不含卡组快照（免得回收时把别人的卡面删花），这边卡组快照正是必须找出来的副本
- `payment/play.service.js` - **Google Play 结算**：`sweepUnconsumed` 清扫器（每分钟，0 号实例）把没跑完的订单接着跑完 —— consume 重试（测试购买 60 秒快车道：3 分钟不 acknowledge 就会被 Google 自动退款）与「崩在抢到发币权与币真的进账之间」的补发（按**账本**里那条带订单号的 recharge 判断发没发）；服务账号 JWT 换 access token（不引 google-auth-library，十几行的事）、`purchases.productsv2` 查验（⚠ `quantity`/`productId`/`consumptionState` 在 `productLineItem[].productOfferDetails` 里，**不在顶层**）、发币（抢占条件带 `revokedAt: null` —— 退款通知可能先于兑换到达）、consume（在发币之后；它蕴含 acknowledge）、退款回收（`revokedAt` 条件原子抢占，RTDN 与每小时轮询必然重叠）、部分退款按 `voidedQuantity/quantity` 比例（**分母取订单快照**，API 那边的 refundableQuantity 退款后已经变了）。⚠ **不走 order.service.applyCallback**：Play 回包里没有任何金额字段，那里的 `paidFen < amountFen` 会把每一笔真实购买标成 failed
- `billing.service.js` - **「钱」的序列，唯一实现**：冻结（退款欠额）→ 套餐门禁 → **每日上限** → 原子扣 → 转发 → 没受理退 → 管理员免单记账。`chargedCall` 给非流式链路一把梭；流式链路（陪聊 / 客服 / 试聊）用 `preAuthorize` + `refundUnaccepted` 两段（SSE 是边跑边写的，包不进 forward，而「先扣再转发」不能让步）。`settleOverCharge` 给 ASR 做「预扣上界、按真实时长多退」
- `chatMemory.service.js` - 数字人对话的「记忆」，**两条聊天链路与 `/api/chat` 只调这里**：会话开 / 存（`beginTurn` 先存用户那句，`finishTurn` 存回复——半截的也存、标 partial——并记用量）；上下文组装 `buildContextMessages`（摘要与原文起点出自同一份会话文档；system 末尾接记忆块：事实卡 + 摘要、每条压成一行，客服那块写明不得覆盖上面的规则；最多取最近 400 条原文，连续同角色合并且一条 ≤4000 字；超预算从最老的原文线性兜底裁剪）；用量 `contextState`（上一轮接口 usage 的 prompt + completion ÷ 产品预算：陪聊 `COMPANION_CTX_BUDGET` 默认 32000、客服 `SUPPORT_CTX_BUDGET` 默认 16000；≥60% warn、≥75% 回复后异步自动提纯、连续两次提纯失败 full）；提纯 `compactThread`（抢带时限的提纯租约 → `aiComplete` 出 JSON：摘要 + 事实卡增 / 改 / 删，敏感信息过滤（NFKC 后按模式认号码，允许段间夹空格 / 横线 / 破折号 / 斜杠 / 逗号；变形邮箱在提到「邮箱」时拦）；切口落在轮次边界、一次最多压 4 万字，保留最近 6 轮陪聊 / 4 轮客服原文，手动整理只留 1 轮；自动提纯至少要压掉 2 轮才调模型；等模型期间会话被删则什么都不写，收尾时发现会话没了就把这次写进去的卡清掉）；硬删与 `DeletionLog`（DeletionLog 先写；删会话一律**先删会话本体**，同时在写的消息 / 提纯以它为准自清；删到一半出错再删一次能续做完）；`purgeUserChatData`（三条删账号入口都在删 User 之前调它）；`sweepExpiredChats` 惰性过期清扫（每进程 10 分钟一轮，删前再核 lastActiveAt、跳过正持有提纯租约的；客服的记忆卡随会话删，陪聊的保留）。token 估算：汉字 0.6、其余 0.3，用接口 usage 滑动校准
- `companionSetting.service.js` - 数字人三项选择的读取 / 合并 / 写入：人格 = 用户选的 → 模型作者推荐的 → 默认；音频 = 用户覆盖 > 人格自带 > 模型推荐 > `COMPANION_TTS_VOICE`（声音身份 `mix` / `voiceId` / `templateId` **整体**取自第一个带身份的层，rate / pitch / instruct 逐字段，见 `utils/voiceSettings.js`）；`PUT /settings` 的 voice 可以只给 `{ templateId }`，经 `voiceTemplate.service.expandVoiceInput` 从模板展开成快照；`personaPromptLine` 给两条聊天链路的系统提示词加一段【人设】（只改语气措辞，客服的依据/红线/转人工规则不动）
- `voiceTemplate.service.js` - 声音市场的序列化（`toVoiceTemplatePayload`，含 `voice` 快照）、`expandVoiceInput`（收 VoiceSettings 的三个写入口——数字人设置 / 人格 / Live2D 模型——共用：完整快照照 normalize，只给 `{ templateId }` 就从模板展开，显式 rate / pitch / instruct 覆盖模板的；模板不存在 404、私有他人 403）、`detachTemplateEverywhere`（删模板时把 CompanionSetting / Persona / Live2dModel 里的 `voice.templateId` 置 null，配方保留）
- `personaAccess.service.js` - 「这个用户能不能选用这个人格」的唯一判定（公开或自己的；付费须有已结算的 PersonaPurchase）+ 数字人接口用的人格摘要（含 styleDescriptor、greeting、examples、voice）+ `styleDescriptorOf`（人设文本的**唯一**拼法：名字｜风格｜语气｜口头禅｜倾向｜称呼用户｜边界，≤900 字；persona.controller.computeStyleDescriptor 只是它的别名）
- `live2dBundle.service.js` - Live2D 模型包的接收（multer 25MB zip）、安全解压（扩展名白名单、zip-bomb 记账、路径穿越剥离）、入口识别（多个 model3.json 时按名字排序 / 上传者指定 `entry`）、model3.json 校验（Cubism 4 + 引用文件存在 + moc3 文件头 `MOC3`）、`inspectBundleBuffer`（解到 `uploads/tmp-inspect/` 只看不存）、`reinspect`（老数据补能力档案）、`writeCompanionJson`（映射落成 model3.json 旁边的 companion.json）、`downloadDirectBundle`/`destroyDirectBundle`（签名直传上来的 zip：走公开投递地址取回、用完 destroy，raw 的 public_id 带 .zip）、目录删除；组件中心与模型市场共用
- `directUpload.service.js` - **Cloudinary 签名直传「一张票」的唯一实现**（铁律六）：`signDirectUpload`（public_id 服务端签死 / overwrite:false / allowed_formats 三条纪律与它们各自挡住的实测漏洞）、`cloudinaryReady`、`directDownloadUrl`（**签名下载**地址：raw 的公开投递在本账号上是 401，2026-09-07 线上实测，只有 `api.cloudinary.com/.../raw/download` 这条签名链接回 200；本地算签名，仍然不烧 Admin API 的 500 次/小时配额）、`DIRECT_UPLOAD_CHUNK_BYTES`。三个上传口共用：模板视频 / 发布成片 / Live2D 模型包
- `live2dCapabilities.service.js` - 第三方 Live2D 模型「会什么」：`extractCapabilities`（动作组 / 表情 / 命中区 / 参数（cdi3）/ 物理 / 贴图尺寸 → `Live2dModel.capabilities` + 市场角标）、`suggestMapping`（nizima 规范名 / 官方 mascot 组名 / 旧式参数 id → 我们的动作 / 表情 / 触摸 / 参数槽，对不上留 null）、`validateMapping`（用户提交的 companion.json：槽位名只能是我们认识的，引用的动作组 / 表情 / 命中区必须在包里）、`completenessOf`；**自动映射只有这一处实现**，槽位名镜像 `companion.service` 的 FACES / ACTIONS 与两端 protocol.ts 的 TOUCH_AREAS
...
| 2026-09-24 | — | **陪聊的自伤危机协议 + AI 身份告知 + 首次同意**（加州 SB 243 §22602/§22603/§22604、纽约 GBL §1701/§1702；方案见私密文档《区域分流与供应商方案》§8）：新增 `services/chatSafety.service.js` 与 `models/SafetyReferralStat.js`；`/api/companion/chat` 输入命中 → **不调模型**（0 token）、求助卡进历史（`ChatMessage.kind='safety'`）、发 `safety` 事件（老客户端退化成一句 `sentence`）；`streamCompanionReply` 的**输出侧守卫默认开**（`guard: false` 才关，目前没有该关的链路）：逐句检查，命中即不发该句、abort 上游、改发求助卡、记一次匿名计数，被拦内容不进历史，开守卫时不再发未经检查的 `token` 事件 —— 检测与计数都落在这一处，**新增聊天链路无需改动即受保护**（自审时发现旧写法 `{messages[]}` 与人格试聊两条链路原本一道闸都没有，靠调用方自己记得传必然会漏）；输入侧检测覆盖 `/api/companion/chat` 的两种请求体与 `/api/personas/preview-chat`；求助卡的事件形态（新客户端 `safety` / 老客户端退化成同形 `sentence`）与 SSE 响应头各只有一处实现（`companion.crisisCardEvents` / `companion.openSse`）；系统提示词加两条不可覆盖的安全底线（承认自己是 AI、不谈自伤细节）并在 few-shot 后重发一次，小梦人设从「16 岁左右」改为成年；AI 身份告知 `notice`（新会话 / 空闲 30 分钟 / 每 3 小时，记在 `ChatThread.safety.lastDisclosureAt`）；首次同意（`CompanionSetting.consent` + `PUT /api/companion/consent` + `config.safety`，`COMPANION_REQUIRE_CONSENT=1` 时未同意返回 428）；危机内容不进上下文（换固定占位句）、不进提纯、不进记忆卡。测试 `tests/chatSafety.spec.js`（14），8 个变异全被抓到。 |
| 2026-09-24 | — | **NCII 移除通道（TAKE IT DOWN Act §3，2026-05-19 起 FTC 执法、无小企业豁免）**：新增 `models/TakedownRequest.js`、`services/nciiTakedown.service.js`、`routes/takedown.routes.js`；`POST /api/takedown` **免登录**（受害者通常不是我们的用户），按 §3(b)(1)(A) 校验四个要件（电子签名 / 内容位置 / 好意相信未经同意 / 联系方式），缺一不成立；`dueAt` 由 `receivedAt + 48h` 推导且不接受传值；收到即发邮件给管理员（`TAKEDOWN_NOTIFY_EMAIL`），剩 12 小时与逾期各再发一封（0 号实例、15 分钟一轮）；管理端 `scan` 检索已知相同副本（22 张表的登记表 + 作品页链接展开 + Cloudinary 变换地址按文件名匹配），处置留痕 `removed[]`。站内举报加 `ncii` 理由并进 `URGENT_REASONS`（插队 + 不随删号级联清掉），⚠ 它**不是**法定通道、不产生 48 小时计时。官网 `/takedown` 公示流程，页脚与首页都有入口。⚠ **上线前必须填 `TAKEDOWN_NOTIFY_EMAIL`**：没有收件人时请求只会在日志里留一行 error。测试 `tests/nciiTakedown.spec.js`（19），8 个变异全被抓到。 |
| 2026-09-24 | — | **Google Play 结算（服务端）**（方案 §12 / §15.5）：新增 `config/play.js`（商品表；★ sku 必须与 Play Console 逐字相同）、`services/payment/play.service.js`、`POST /api/pay/play/redeem`（幂等，靠 `TokenOrder.playPurchaseToken` 唯一索引兜底）、`POST /api/pay/play/rtdn?key=…`（Pub/Sub 推送；密钥不对**一律 404**，任何错误都回 200 否则会被无限重推）、每小时拉一次 voidedpurchases（0 号实例，RTDN 的兜底）。`TokenOrder` 加 `playPurchaseToken`(unique partial) / `playOrderId` / `quantity` / `isTest` / `grantedAt` / `consumedAt` / `revokedAt` / `clawbackTokens` / `shortfall` / `voidedQuantity` / `refundType` 与 `refunded` 状态，`user` 改为可空（退款先于兑换到达时要先占位）；`User` 加 `playAccountId` / `playRefundCount`。测试 `tests/playBilling.spec.js`（22），10 个变异全被抓到。⚠ **香港源站到 androidpublisher/pubsub/oauth2 的连通性已实测**（2026-09-24 在生产机上 curl：discovery 200、productsv2 401 鉴权缺失 —— 说明路由通、TLS 通、API 可达）。 |
| 2026-09-24 | — | **四条不计费链路接进钱包 + 退款欠额**（方案 9.1 的 R1.5 与 §15.4）：① 抽出 `services/billing.service.js` —— 钱的序列此前只长在 `chargedArkCall` 里，于是「不走方舟就不扣费」；② `priceOf` 补 **`tts`（33 token/字符）/ `asr`（5,000 token/分钟，按字节上界预扣、按上游真实时长多退）/ `runway`（按秒，单价回溯 Runway 自己的 credit 价目页）** 三种 kind；③ `tts.routes` / `asr.routes` / `runway.routes` / 陪聊 / 客服 / **人格试聊**全部接进钱包（此前这几处 `priceOf`/`charge`/`wallet` 零命中，单账号理论日上限 $1,250/日 + ¥2,160/日）；④ **每日 token 上限**（`DAILY_LIMITS`：免费 150k、付费硬线 3M），判据是「**已花 ≥ 上限**」而不是「已花 + 本次 > 上限」—— 后者会让一段 30 秒的 seedance-2.5（3,045,600）永远点不动，正是 §14.10 自己点名的死配置形状；⑤ Runway 查不到价**一律 501，绝不降级成免费**；⑥ 退款欠额 `tokenWallet.debt` 与冻结（403 `WALLET_FROZEN`）。测试 `tests/billing.spec.js`（23，含一条**扫路由与控制器**的登记册自检 —— 这次真的靠它逮到漏接的人格试聊）+ `tests/walletDebt.spec.js`（21）；16 个变异全被抓到。⚠ 连带改了四个既有套件的夹具：它们用同一个账号连发几十发贵调用，会撞新的日上限，每条用例先清当天流水。 |
| 2026-09-18 | — | **数字人对话记忆 L1+L2（服务器）**（设计见 app 仓 `docs/character-art-privacy-context.md` §B / §C）：新增 `ChatThread` / `ChatMessage` / `ChatMemory` / `ChatUsageLog` / `DeletionLog` 五个模型、`services/chatMemory.service.js`、`routes/chatThreads.routes.js`（`/api/chat`）；`/api/companion/chat` 与 `/api/support/chat` 加 `{ message, threadId? }` 按会话写法（历史服务端持有、`thread` 事件、`done` 带 `threadId` + `context`、回复后按用量自动提纯），旧 `{ messages[] }` 写法原样保留；`aiClient.aiChatStream` 带 `stream_options.include_usage` 并以 `opts.onUsage` 交出用量（端点不认就去掉重发，可与 max_completion_tokens 换名叠加），`aiComplete` 多返回 `usage`、收 `opts.maxTokens`，新增 `normalizeUsage`（DeepSeek / OpenAI 两种 usage 字段）；`streamCompanionReply` 加 `thread` / `finish` 两个可选钩子，并修两处老毛病（两条聊天链路都修）：客户端在开流前就断开时直接 abort（此前 `close` 已错过，上游照跑完、token 照扣）；上游半路出错时把缓冲里的半句静默收进正文。删账号的三条入口（`branchAdmin.purgeUserCascade` ⑩.5，回执多 `chatThreads` / `chatMemories`；`users.deleteAccount`；admin 删用户）连带硬删对话数据。切句器收尾丢掉被截断的半个标签。请求体的 `threadId` 可为 null（等同不给）。`.env.example` 加 `COMPANION_CTX_BUDGET` / `SUPPORT_CTX_BUDGET`。测试：新增 `tests/chatMemory.spec.js`（49）、`tests/aiClientUsage.spec.js`（8）。 |
| 2026-09-08 | — | **修：回炉的三条不可逆路径**（#62 合并前的聚焦复核挖出，三位评委一致 do-not-merge；三条都只在回炉链路上，而回炉要客户端带 segments/branchTree/deck 才触发 ⇒ App 未发版时线上够不到）。① `transferAssetsFor` 现在把转存账目回给调用方，回炉在写库**之前**判 `failed > 0` 就整句拒（502 `REVISE_TRANSFER_FAILED`）—— 此前失败被静默降级成方舟临时链接或空串，而 ⑤ 的差量回收随后把**旧的那条永久资产**当 gone 删掉，接口还回 200、弹幕已清、版次已涨、通知已发。② 409 分支的孤儿回收把 in-use 方向用反了：`assetInUseByOthers` 用 `_id: {$ne: 本条}` 排除自己，而 409 的成因恰恰是「另一发已经把这批新地址写进了同一条作品」⇒ 会把**赢家刚上线的成片**当孤儿 destroy。新增 `keepUrlsOf(before, now)`（keep 集的唯一实现）把库里当下那份 assetUrls 并进来。③ `scripts/backfillAssetUrls.js` 的 bulkWrite 只按 `_id` 定位：这个脚本按设计是**服务端在线时**跑的，读与写之间运行时写过的 assetUrls 会被旧正文整份覆盖，而覆盖之后 `$exists` 判据照样是 0（**假的全绿**）。更新条件补上 `assetUrls: {$exists:false}`，新增 `--verify` 模式（比对 stored 与按正文现算的集合），`buildOp` 提成可导出的唯一判据。测试：新增 `tests/backfillAssetUrls.spec.js`（4 条）与 R10 / R11a / R11b，三条都做过**负向验证**（退回旧写法必红）。 |
| 2026-09-07 | — | **门禁收口（铁律六）**：上一行那次改动里 `branchVideo.controller.js` 与 `branchProject.controller.js` 各写了一份逐字相同的 `failWith(status, code, message, details)`（带**自定义 code + 整句中文**的 4xx，`utils/http` 里那几个的 code 是 `errorCodes` 固定枚举、兜不住 `REVISE_*` / `PROJECT_*`）。合并进 `utils/http.js` 并导出，两个控制器改成 import；随之两边的 `require("../utils/AppError")` 都成了死 import，一并删掉。行为零变化（`branchRevise` / `branchProject` / `branchVideoVisibility` / `branchDelete` / `branchDanmaku` / `report` / `security` 全绿）。 |
| 2026-09-07 | — | **已发布作品的「回炉重做」（服务端）**（设计见 app 仓的特性 B 规格）：新增 `models/BranchProject.js`（画布快照，独立集合、按 `video` 唯一、不加 TTL）、`schemas/branchProject.schemas.js`（`NO_LOCAL` 不变量 + 2MB/100 条/50MB 三个**拍出来的**阈值）、`routes/branchProject.routes.js`、`controllers/branchProject.controller.js`、`scripts/backfillAssetUrls.js`（`npm run backfill:asset-urls`）。`BranchVideo` 加 `revision` / `revisedAt` / `assetUrls`（必须进 schema：mongoose strict 会把 `$inc` 静默丢掉）。`PATCH /api/branch/videos/:id` 扩成**唯一的内容写入通道**：`updateBody` 加 `segments` / `branchTree` / `deck` / `baseRevision`（**仍然绝不 `.loose()`**，takedown 那道安全门不动），带了内容字段即回炉——四道 400（`REVISE_PAID` / `REVISE_TAKEN_DOWN` / `REVISE_LOCKED` / `REVISE_NO_BASE`）、乐观并发 409（`REVISE_CONFLICT` + `details.currentRevision`）、资产差量回收（新增 `assetInUseByOthers` **唯一实现**，`purgeVideo` 与回炉共用——此前 `purgeVideo` 一道 in-use 判定都没有，回炉之后新旧两版共享资产会变成常态，删一条打死另一条）、清空该作品弹幕、`BRANCH_REVISED` 通知收藏者（并发 8、上限 500、进 `NOTIF_DEDUP_KEYS` 去重）。`transferDraftAssets` 重构成 `transferAssetsFor`（**只处理真的带了的键**，顺带修掉无条件读 `draft.segments.length` 的 500）；`toVideoPayload` 加 `revision` / `revisedAt`；`purgeVideo` 与 `purgeUserCascade` 双双级联删 `BranchProject`；PATCH 挂上限流 `branch:revise` 6/分钟（此前一条限流都没有）；举报复核的作品对象带上 `revision` / `revisedAt`（**不是**内容快照）。⚠ **上线顺序**：服务端上线 → 立刻跑 `backfillAssetUrls` 并确认 `countDocuments({ assetUrls: { $exists: false } }) === 0` → 才发 App（漏了这一步，删一条新作品会 destroy 掉老互动作品分支段还在用的资产，老作品当场黑屏、零报错）。⚠ 老 App（≤v2.45）**收不到** `BRANCH_REVISED`（它的 `BRANCH_NOTIFICATION_TYPES` 是请求层白名单）——对未升级用户观众知情为零，不是"降级显示"。测试：新增 `tests/branchProject.spec.js`、`tests/branchRevise.spec.js`；`tests/branchVideoVisibility.spec.js` 里那条「片段与卡组改不动」改写成四条（非作者 / 付费 / 已下架 / **takedown 仍被 strip**）并留在原 describe 里。 |
| 2026-09-07 | — | **修：直传的 raw 包取不回来**（合并 #60 后第一次真机跑直传就撞上）。`downloadDirectBundle` 原来走公开投递地址，而本账号对 raw 的**公开投递是关的**：实测公开地址 401、带签名的投递地址 401、Admin API 回的 secure_url 也 401，只有 `private_download_url` 生成的 `api.cloudinary.com/v1_1/<cloud>/raw/download?...signature` 回 200（822,693 字节，与设备上传的原文件逐字节相等）。改走签名下载地址（public_id 必须带扩展名、format 传空串 —— 拆成 base+"zip" 实测 404）。★ 这一条 jest 抓不到：spec 里 axios 是 mock 的，契约全绿也证明不了投递域名放不放行。 |
| 2026-09-07 | — | **Live2D 模型包签名直传**（创作中心 P3 的服务端前置）：抽出 `services/directUpload.service.js` 作为签名直传票的唯一实现，模板视频 / 发布成片两处逐字重复的签名代码收敛到它（行为不变，`tests/uploadsMediaDirect.spec.js` 与 `tests/branchTemplate.spec.js` 钉住）；`utils/templateVideoAsset.js` 加 `LIVE2D_BUNDLE_FOLDER` / `ownLive2dBundlePublicId`（**回带 .zip** —— raw 资产的 public_id 含扩展名，destroy 与投递地址都要用它）；`POST /api/live2d-models/bundle/sign` 出票，`/inspect` 与 `POST /api/live2d-models` 收 `bundleRef`（+ `bundleName`），取回走公开投递地址而非 Admin API，体积闸在服务层补回（直传路上 multer 不在链上），create 在 finally 里回收那份 raw 资产。测试：新增 `tests/live2dBundleSign.spec.js`。⚠ 已知：`tests/uploadsMediaDirect.spec.js` 在本机（Windows）beforeAll 注册用户时报 MongoNotConnectedError 全挂，在 origin/main 的干净工作树上同样复现，与本次改动无关。 |
| 2026-09-05 | — | **数字人创作中心 P1（服务器）**（设计见 app 仓 `docs/digital-human-creator-center.md`）：① 人物模型——新增 `services/live2dCapabilities.service.js`（能力档案提取 / 自动映射 / 映射校验），`Live2dModel` 加 `capabilities` / `mapping` / `license` / `takenDown`，`POST /api/live2d-models/inspect`（只看不存），上传 / 修改收 `mapping`（写成包内 `companion.json`，运行时按相对路径读；老包没有此文件行为不变）/ `entry` / `selfMade`，`inspectModel3Json` 加 moc3 文件头校验，`findModelEntryFile` 优先 model3.json 并支持指定入口；下架过滤进列表 / 详情 / `loadUsableModel`。② 人物人格——`Persona.style` 加 `tone` / `addressUser` / `greeting` / `examples` / `boundaries`（`styleDescriptorOf` 一并拼进，上限 900），`Persona` 加 `takenDown`；`POST /api/personas/analyze`（素材分析）、`POST /generate` 扩参（basics / questionnaire / analysis / only+draft，向后兼容 chatText）、`POST /preview-chat`（草稿试聊 SSE）；`listPersonas` 改数据库分页 / 搜索 / 排序（此前全量 find 再 JS 过滤）；`companion.service` 抽出 `streamCompanionReply` 与 `personaExampleMessages`，首页看板娘与客服聊天都把人格示例对话当 few-shot 注入；`persona.controller.computeStyleDescriptor` 改为 `personaAccess.styleDescriptorOf` 的别名（此前两份实现）。测试：新增 `tests/live2dCapabilities.spec.js`、`tests/personaWizard.spec.js`；`tests/live2dModel.spec.js` 的假 moc3 补 `MOC3` 头。 |
| 2026-09-04 | — | **声音市场（豆包 1.0 混音模板）**：`config/voices.js` 加 `MIXABLE_VOICES`（23 个 2026-09-04 逐个验证过能出声的 1.0 音色，分性别）/ `isMixableVoice` / `MAX_MIX_VOICES`，2.0 目录每条带 `generation` / `mixable`；`utils/voiceSettings.js` 的 VoiceSettings 加 `mix`（1～3 味 1.0 音色，权重归一到和 = 1、三位小数，2.0 进来 400 且 message 说明只能混 1.0）与 `templateId`，合并改为「声音身份整体取自第一个带身份的层」；`CompanionSetting.voiceSettingsSchema` 子文档同步加 `mix` / `templateId`（Persona / Live2dModel 复用）；新增 `VoiceTemplate` + `VoiceTemplateLike` 与 `/api/voice-templates`（列表 / 详情 / 创建 10 次/分钟 / 改 / 删 / like / use）、`services/voiceTemplate.service.js`（`{ templateId }` → 快照展开，数字人设置 / 人格 / 模型三个写入口共用；删模板只把引用方的 `templateId` 置 null）；`GET /api/tts/voices` 多出 `mixable` / `maxMixVoices`，`POST /api/tts` 的 `mix` 同时收 `{ id, w }` 与 `{ voiceId, weight }`（≤3 味）；`middleware/error.js` 对 ZodError 改读 zod 4 的 `issues`（此前 `details` 一直是 undefined）并把 superRefine 的 custom 人话当 message。测试：新增 `tests/voiceTemplate.spec.js`、`tests/tts.spec.js`（mock fetch），`tests/companionSetting.spec.js` / `tests/live2dModel.spec.js` 补混音合并与模板展开。 |
