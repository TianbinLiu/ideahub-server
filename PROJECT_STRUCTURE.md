...
- `ChatThread.js` - 数字人对话的一个会话（scene = companion 首页陪聊 | support App 客服，两边互不可见）：标题、原子序号 `seq`、`lastActiveAt`（保留期从这里算：陪聊 180 天、客服 30 天，惰性清扫）、滚动摘要 `summary`（≤600 字 + 上一版可回退）、上下文计量 `stats`（上一轮接口 usage / 估算校准系数 calibK / 连续提纯失败次数 / 提纯锁）
...
- `chatThreads.routes.js` - 数字人对话的会话与记忆（陪聊 + 客服共用，base `/api/chat`，全部需登录、只碰自己的）：`GET /threads?scene=` 会话列表（带 context 用量）、`GET /threads/:id/messages?before=&limit=` 翻历史、`POST /threads/:id/compact {focus?}` 手动「整理记忆」（调模型，按用户 5 次/分钟；正在整理 409、失败 502）、`POST /threads/:id/summary/revert`、`DELETE /threads/:id` / `DELETE /threads?scene=`（硬删，连带从它提炼的记忆卡）、`GET|DELETE /memories?scene=`、`PATCH /memories/:id {text?, pinned?}`、`POST /memories/:id/revert`、`DELETE /memories/:id`
...
- `chatMemory.service.js` - 数字人对话的「记忆」，**两条聊天链路与 `/api/chat` 只调这里**：会话开 / 存（`beginTurn` 先存用户那句，`finishTurn` 存回复——半截的也存、标 partial——并记用量）；上下文组装 `buildContextMessages`（system 末尾接记忆块：事实卡 + 摘要，客服那块写明不得覆盖上面的规则；连续同角色合并；超预算从最老的原文兜底裁剪）；用量 `contextState`（上一轮接口 usage 的 prompt + completion ÷ 产品预算：陪聊 `COMPANION_CTX_BUDGET` 默认 32000、客服 `SUPPORT_CTX_BUDGET` 默认 16000；≥60% warn、≥75% 回复后异步自动提纯、连续两次提纯失败 full）；提纯 `compactThread`（抢 `stats.compacting` 锁 → `aiComplete` 出 JSON：摘要 + 事实卡增 / 改 / 删，敏感信息过滤，保留最近 6 轮陪聊 / 4 轮客服原文，手动整理只留 1 轮；自动提纯至少要压掉 2 轮才调模型）；硬删与 `DeletionLog`；`purgeUserChatData`（删账号用，待接入）；`sweepExpiredChats` 惰性过期清扫（每进程 10 分钟一轮，照 assetPurge 的成方）。token 估算：汉字 0.6、其余 0.3，用接口 usage 滑动校准
...
| 日期 | 版本 | 更新内容 |
|------|------|---------|
| 2026-09-18 | — | **数字人对话记忆 L1+L2（服务器）**（设计见 app 仓 `docs/character-art-privacy-context.md` §B / §C）：新增 `ChatThread` / `ChatMessage` / `ChatMemory` / `ChatUsageLog` / `DeletionLog` 五个模型、`services/chatMemory.service.js`、`routes/chatThreads.routes.js`（`/api/chat`）；`/api/companion/chat` 与 `/api/support/chat` 加 `{ message, threadId? }` 按会话写法（历史服务端持有、`thread` 事件、`done` 带 `threadId` + `context`、回复后按用量自动提纯），旧 `{ messages[] }` 写法原样保留；`aiClient.aiChatStream` 带 `stream_options.include_usage` 并以 `opts.onUsage` 交出用量（端点不认就去掉重发，可与 max_completion_tokens 换名叠加），`aiComplete` 多返回 `usage`、收 `opts.maxTokens`，新增 `normalizeUsage`（DeepSeek / OpenAI 两种 usage 字段）；`streamCompanionReply` 加 `thread` / `finish` 两个可选钩子。`.env.example` 加 `COMPANION_CTX_BUDGET` / `SUPPORT_CTX_BUDGET`。测试：新增 `tests/chatMemory.spec.js`（26）、`tests/aiClientUsage.spec.js`（8）。 |
