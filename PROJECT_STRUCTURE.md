...
| 日期 | 版本 | 更新内容 |
|------|------|---------|
| 2026-09-18 | — | **数字人对话记忆 L1+L2（服务器）**（设计见 app 仓 `docs/character-art-privacy-context.md` §B / §C）：新增 `ChatThread` / `ChatMessage` / `ChatMemory` / `ChatUsageLog` / `DeletionLog` 五个模型、`services/chatMemory.service.js`、`routes/chatThreads.routes.js`（`/api/chat`）；`/api/companion/chat` 与 `/api/support/chat` 加 `{ message, threadId? }` 按会话写法（历史服务端持有、`thread` 事件、`done` 带 `threadId` + `context`、回复后按用量自动提纯），旧 `{ messages[] }` 写法原样保留；`aiClient.aiChatStream` 带 `stream_options.include_usage` 并以 `opts.onUsage` 交出用量（端点不认就去掉重发，可与 max_completion_tokens 换名叠加），`aiComplete` 多返回 `usage`、收 `opts.maxTokens`，新增 `normalizeUsage`（DeepSeek / OpenAI 两种 usage 字段）；`streamCompanionReply` 加 `thread` / `finish` 两个可选钩子，并修两处老毛病（两条聊天链路都修）：客户端在开流前就断开时直接 abort（此前 `close` 已错过，上游照跑完、token 照扣）；上游半路出错时把缓冲里的半句静默收进正文。删账号的三条入口（`branchAdmin.purgeUserCascade` ⑩.5、`users.deleteAccount`、admin 删用户）连带硬删对话数据。`.env.example` 加 `COMPANION_CTX_BUDGET` / `SUPPORT_CTX_BUDGET`。测试：新增 `tests/chatMemory.spec.js`（40）、`tests/aiClientUsage.spec.js`（8）。 |
